# №38 Дополнить телефонный справочник возможностью изменения и удаления данных. Пользователь также может ввести имя или фамилию, и Вы должын  реализовать функционал  для изменения и удаление данных
...
class Contacts:
    def find_member(self, query):
        with open('Homework2-8/data.txt', encoding='utf-8') as file:
            for line in file:
    ...
    def add_member(self):
        ...
        if c.find_member(query=(m.last_name, m.name)) is None:
            f = open('Homework2-8/data.txt', 'a', encoding='utf-8')
    ...
    def delete_member(self, query):
        objects = []
        f = open('Homework2-8/data.txt', 'r+', encoding='utf-8')
